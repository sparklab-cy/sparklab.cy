<script lang="ts">
	...
	import type { CustomCourse, Lesson, LessonFile, UserProgress } from '$lib/types/courses';

	const { data } = $props();
	const { course, userProgress, error } = data;
	...
	const courseTyped = course as CustomCourse & { creator?: { full_name: string | null } };
</script>
	...
	{:else if course}
		<header class="viewer-header">
			...
			<div class="header-center">
				<h1 class="course-title">{courseTyped.title}</h1>
				<p class="course-desc">{courseTyped.description}</p>
				<span class="creator-line"
					>By {courseTyped.creator?.full_name ?? 'Teacher'}</span
				>
			</div>
			...
			<main class="viewer-main">
				...
				{:else}
					<div class="no-lesson-selected">
						<p>Select a lesson from the list, or wait for the teacher to add content.</p>
					</div>
				{/if}
			</main>
		</div>
